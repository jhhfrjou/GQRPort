...
class GQRTest {
    ...
    @BeforeEach
    void setUp() {
        gqr = new GQR(new File("resources/query_0.txt"),new File("resources/views_for_q_0.txt"), 0);

    }

    @Test
    void getQuery() {
        DatalogQuery query = gqr.getQuery();
    ...
    void reformulateFail() {
        gqr = new GQR(new File("resources/paper_query.txt"),new File("resources/paper_views.txt"),0);
        try {
            gqr.reformulate(gqr.getQuery());
            assertThrows(NonAnswerableQueryException.class,null);
        } catch (NonAnswerableQueryException e) {
